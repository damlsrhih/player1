...
public:
    ...
    //get系列方法，通过常量方法的常量修饰进行重载（重载同名的读、写方法）
    ...
    //输出到文件，使用QTextStream
    friend QTextStream& operator<<(QTextStream& stream, const Song& song);
};

/*歌曲对象管理类，管理运行时添加的所有歌曲构造的歌曲对象
 *  实现为单例类
 *  包含一个歌曲对象列表，存储<路径，歌曲对象指针>键值对容器，注意释放问题
 *  返回歌曲对象列表的接口
 *  查询歌曲对象数量size
 *  清空歌曲接口
 *  是否包含某首歌接口
...
public:
    ...
    //清空歌曲接口
    void clear() { for (auto song : m_songs) { if (song) { delete song; }} }
